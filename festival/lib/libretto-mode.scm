...
(xml_register_id "-//LIBRETTO//DTD LIBRETTO mark up//EN"
		(path-append libdir "Libretto_001.dtd")
		)

(xml_register_id "-//LIBRETTO//ENTITIES Added Latin 1 for LIBRETTO//EN"
...
;; parsing each of our XML tags: LIBRETTO, PITCH, DURATION, and REST.
;;
...
;; get-bpm
;;
;; Given the attribute list of a LIBRETTO tag, returns the beats
...
;; seconds using BPM, which was set in the LIBRETTO tag.  If this
...
(define (get-durs atts)
  ...
  (set! beats (car (car (cdr (assoc 'BEATS atts)))))
    (if (or (equal? beats nil) (equal? beats 'X)) ;(equal? beats 'X)
...
(define (singing_f0_targets utt syl)
  ...
  (let (   
        (freq  (parse-number (syl2freq syl)   ) )
